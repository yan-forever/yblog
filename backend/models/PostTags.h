...
class PostTags
{
  public:
    ...
    using PrimaryKeyType = std::tuple<int32_t,int32_t>;//post_id,tag_id
     ...
     */
    explicit PostTags(const drogon::orm::Row &r, const ssize_t indexOffset = 0) noexcept;

    /**
     * @brief constructor
     * @param pJson The json object to construct a new instance.
     */
    explicit PostTags(const Json::Value &pJson) noexcept(false);
     ...
     */
    PostTags(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false);
  ...
  private:
    friend drogon::orm::Mapper<PostTags>;
    friend drogon::orm::BaseBuilder<PostTags, true, true>;
    friend drogon::orm::BaseBuilder<PostTags, true, false>;
    friend drogon::orm::BaseBuilder<PostTags, false, true>;
    friend drogon::orm::BaseBuilder<PostTags, false, false>;
#ifdef __cpp_impl_coroutine
    friend drogon::orm::CoroMapper<PostTags>;
#endif
  ...
  public:
    ...
    {
        static const std::string sql="select * from " + tableName + " where post_id = ? and tag_id = ?";
        return sql;
    }

    static const std::string &sqlForDeletingByPrimaryKey()
    {
        static const std::string sql="delete from " + tableName + " where post_id = ? and tag_id = ?";
        return sql;
    }
